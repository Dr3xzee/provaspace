...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadReferralSettings() {
        ...
        const val = snap.exists() ? (snap.data().rentDays ?? snap.data().rentCredit ?? 0) : 0;
    ...
    function wireReferralForm() {
        document.getElementById('saveReferralBtn')?.addEventListener('click', async () => {
            const days = parseFloat(document.getElementById('referralRentCredit').value) || 0;
            try {
                await setDoc(doc(db, 'settings', 'referral'), { rentDays: days, rentCredit: days, updatedAt: new Date() }, { merge: true });
                alertModal('Saved', `Referral reward set to ${days} free rent day(s) per successful referral.`);
            } catch (err) {
                console.error(err);
