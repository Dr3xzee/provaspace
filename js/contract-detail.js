...
function renderMilestones() {
    ...
    (contract.milestones || []).forEach((m, idx) => {
        ...
        if (m.released) {
            ...
            // Admin can still manually release if needed
            badge = m.releaseRequested
                ? '<span class="badge badge-amber">Client approved</span>'
            ...
            if (!m.submitted) {
                ...
                badge = '<span class="badge badge-green">Payment released ✓</span>';
            } else {
                badge = '<span class="badge badge-blue">Delivered — review & pay</span>';
                // Client releases directly — no admin needed
                action = `<button class="mini-btn" style="font-size:0.7rem;padding:6px 10px;" data-client-release="${idx}">Release Payment</button>`;
            }
        } else {
            // freelancer
            if (!m.submitted) {
                ...
                badge = '<span class="badge badge-blue">Submitted — awaiting client</span>';
            }
        }
    ...
    list.querySelectorAll('[data-client-release]').forEach(btn =>
        btn.addEventListener('click', () => clientRelease(+btn.dataset.clientRelease)));
...
// Client directly releases payment — no admin needed
async function clientRelease(idx) {
    ...
    if (amount > available) {
        ...
        modal(
            ...
            `<p>Only <strong>${fmt(available)}</strong> is in escrow, but this milestone needs <strong>${fmt(amount)}</strong>.</p>
            <p style="margin-top:8px;">Please top up <strong>${fmt(shortfall)}</strong> from your dashboard before releasing.</p>`,
            { confirm: false }
        );
        return;
    }
    ...
    modal(
        'Release Payment',
        `<p>You're about to release <strong>${fmt(amount)}</strong> to the freelancer for "<strong>${esc(m.name)}</strong>".</p>
        <p style="margin-top:8px;font-size:0.8rem;color:var(--accent-red);">This will credit the freelancer's wallet immediately and cannot be undone.</p>`,
        {
            confirm: true, confirmText: '✓ Release Payment',
            onConfirm: async () => {
                const updated = [...contract.milestones];
                updated[idx] = { ...m, released: true, releaseRequested: true, releasedAt: new Date().toISOString(), releasedBy: currentUser.uid };
                const newEarned = (contract.payoutsEarned || 0) + amount;
                const allDone = updated.every(x => x.released);

                await updateDoc(doc(db, 'contracts', contractId), {
                    milestones: updated,
                    payoutsEarned: newEarned,
                ...
                if (clSnap.exists()) {
                    await updateDoc(doc(db, 'users', contract.clientId), {
                ...
                if (allDone) {
                    ...
                    import('./referral-helper.js').then(m => m.checkAndPayReferral(contract.freelancerId)).catch(() => {});
                }
                renderSidebar();
                ...
                await Promise.all([
                    notifyUser(contract.freelancerId, {
                        ...
                        message: `The client released ${fmt(amount)} for "${m.name}" on "${contract.title}".`,
                        type: 'payment_released',
                        link: `contract-detail.html?id=${contractId}`,
                    }),
                ]).catch(() => {});

                showToast(`${fmt(amount)} released to freelancer ✓${allDone ? ' · Contract completed!' : ''}`);
            }
        }
    );
}
...
async function abandonContract() {
    modal(
        ...
        <p style="margin-top:8px;font-size:0.8rem;color:var(--accent-red);">The gig will return to The Space so another freelancer can pick it up.</p>`,
        {
            confirm: true, confirmText: 'Yes, Abandon',
            onConfirm: async () => {
                // Track progress: which milestones were released before abandon
                const milestonesSnapshot = contract.milestones || [];
                const lastReleasedIdx = milestonesSnapshot.reduce((last, m, i) => m.released ? i : last, -1);
                const payoutsEarned = contract.payoutsEarned || 0;

                // Mark contract abandoned
                await updateDoc(doc(db, 'contracts', contractId), { status: 'abandoned', abandonedAt: serverTimestamp(), abandonedBy: currentUser.uid });
                ...
                if (flSnap.exists()) {
                    await updateDoc(doc(db, 'users', currentUser.uid), {
                        ...
                        trustScoreLastReduced: new Date(),
                    });
                }

                // Return gig to The Space with progress context
                if (contract.gigId) {
                    try {
                        const { doc: fDoc, updateDoc: fUpdate, getDoc: fGet } = await import('./firebase.js');
                        const gigRef = fDoc(db, 'gigs', contract.gigId);
                        const gigSnap = await fGet(gigRef);
                        if (gigSnap.exists()) {
                            await fUpdate(gigRef, {
                                status: 'open',
                                claimedBy: null,
                                timerStart: null,
                                previousFreelancer: currentUser.uid,
                                previousPayoutsEarned: payoutsEarned,
                                lastMilestoneReached: lastReleasedIdx >= 0 ? lastReleasedIdx : null,
                                // Mark milestones already paid so next freelancer knows
                                abandonedMilestones: milestonesSnapshot.map((m, i) => ({
                                    name: m.name,
                                    percent: m.percent,
                                    released: m.released,
                                })),
                    ...
                    } catch (e) { console.warn('Could not return gig to space:', e); }
                }

                // Notify client
                notifyUser(contract.clientId, {
                    title: '⚠️ Freelancer abandoned your job',
                    message: `The freelancer abandoned "${contract.title}". The gig has been returned to The Space for re-hire.`,
                    type: 'job_abandoned',
                    link: 'client-dashboard.html',
                }).catch(() => {});
                ...
                renderSidebar();
                showToast('Job abandoned — gig returned to The Space.');
            }
        }
    );
}
