...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('savePricesBtn').addEventListener('click', async () => {
        ...
        const priceData = {
            ...
            assuranceFeeFlat: parseFloat(document.getElementById('assuranceFeeFlat').value) || 0,
            taxPassTiers,
            updatedAt: new Date(),
        };
...
async function loadVerifications() {
    ...
    function trustBadge(score) {
        const s = score ?? 100;
        if (s >= 80) return `<span class="badge badge-green">${s}</span>`;
        if (s >= 50) return `<span class="badge badge-amber">${s}</span>`;
        return `<span class="badge badge-red">${s}</span>`;
    }

    function trustLastReducedLabel(ts) {
        if (!ts) return '<span style="color:var(--text-secondary);font-size:0.78rem;">—</span>';
        const date = ts.toDate ? ts.toDate() : new Date(ts);
        const diffMs = Date.now() - date.getTime();
        const diffDays = diffMs / (1000 * 60 * 60 * 24);
        const label = date.toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' });
        const color = diffDays < 7 ? 'var(--accent-red)' : 'var(--accent-green)';
        const icon = diffDays < 7 ? 'fa-circle-exclamation' : 'fa-circle-check';
        return `<span style="color:${color};font-size:0.78rem;font-weight:600;"><i class="fa-solid ${icon}" style="margin-right:4px;"></i>${label}</span>`;
    }

    function openSetTrustModal(uid, name, currentScore) {
        const box = document.createElement('div');
        box.innerHTML = `
            <p style="margin-bottom:12px;">Set a new trust score for <strong>${escapeHtml(name)}</strong>:</p>
            <input type="number" id="newTrustInput" min="0" max="100" value="${currentScore ?? 100}"
                style="width:100%;padding:10px 14px;border:1.5px solid var(--border-color);border-radius:12px;
                font-size:1rem;font-weight:700;text-align:center;background:var(--bg-main);color:var(--text-primary);">
            <p style="font-size:0.78rem;color:var(--text-secondary);margin-top:8px;">Valid range: 0 – 100</p>
        `;
        showModal('Set Trust Score', box, null);
        modalActionBtn.textContent = 'Save Score';
        modalActionBtn.onclick = async () => {
            const val = parseInt(document.getElementById('newTrustInput').value, 10);
            ...
            modalOverlay.classList.remove('active');
            try {
                const updates = { trustScore: val };
                // Only stamp trustScoreLastReduced if we're lowering the score
                if (val < (currentScore ?? 100)) {
                    updates.trustScoreLastReduced = new Date();
                }
                await updateDoc(doc(db, 'users', uid), updates);
                showModal('Done ✓', `Trust score for ${escapeHtml(name)} set to ${val}.`, () => loadUsers());
            } catch (err) {
                console.error(err);
                showModal('Error', 'Could not update trust score.', null);
            }
        };
    }
    ...
    async function loadUsers() {
        ...
        const lowTrustList = document.getElementById('lowTrustList');
        try {
            const snap = await getDocs(collection(db, 'users'));
            allUsersCache = snap.docs.map(d => ({ id: d.id, ...d.data() }));
            renderUsersTable(allUsersCache);
            renderLowTrustPanel(allUsersCache);
        } catch (err) {
            console.error(err);
    ...
    function renderUsersTable(users) {
        const body = document.getElementById('usersBody');
        body.innerHTML = users.length === 0 ? '<tr><td colspan="6" style="text-align:center;color:var(--text-secondary);">No users found.</td></tr>' : '';
        users.forEach(u => {
            ...
            tr.innerHTML = `
                ...
                <td>${escapeHtml(u.email || '—')}</td>
                <td style="text-align:center;">${trustBadge(u.trustScore)}</td>
                <td>${statusBadge}</td>
                <td>
                    <button class="table-action-btn" style="background:var(--accent-blue);color:#fff;margin-bottom:4px;" data-set-trust="${u.id}" data-name="${escapeHtml(name)}" data-score="${u.trustScore ?? 100}">
                        <i class="fa-solid fa-star-half-stroke"></i> Trust
                    </button>
                    <button class="table-action-btn secondary" data-toggle-suspend="${u.id}" data-current="${u.suspended ? '1' : '0'}">
                        ${u.suspended ? 'Unsuspend' : 'Suspend'}
                    </button>
                    <button class="table-action-btn danger" data-toggle-ban="${u.id}" data-current="${u.banned ? '1' : '0'}">
                        ${u.banned ? 'Unban' : 'Ban'}
                    </button>
                </td>
            `;
        ...
        body.querySelectorAll('[data-set-trust]').forEach(btn => {
            btn.addEventListener('click', () => openSetTrustModal(btn.dataset.setTrust, btn.dataset.name, parseInt(btn.dataset.score, 10)));
        });
        ...
        body.querySelectorAll('[data-toggle-ban]').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                if (!isCurrentlyBanned && !confirm('Ban this user? They will immediately lose access.')) return;
    ...
    function renderLowTrustPanel(users) {
        const lowTrustList = document.getElementById('lowTrustList');
        const lowUsers = users.filter(u => (u.trustScore ?? 100) < 50).sort((a, b) => (a.trustScore ?? 100) - (b.trustScore ?? 100));
        if (lowUsers.length === 0) {
            lowTrustList.innerHTML = '<p style="color:var(--accent-green);font-size:0.88rem;"><i class="fa-solid fa-circle-check"></i> No users with trust score below 50.</p>';
            return;
        }
        lowTrustList.innerHTML = '';
        lowUsers.forEach(u => {
            const name = u.companyName || u.fullName || '—';
            const score = u.trustScore ?? 100;
            const card = document.createElement('div');
            card.style.cssText = `
                background:var(--bg-main);border:1.5px solid var(--accent-red);border-radius:14px;
                padding:14px 16px;display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;
            `;
            card.innerHTML = `
                <div style="display:flex;align-items:center;gap:12px;min-width:0;">
                    <div style="width:38px;height:38px;border-radius:50%;background:var(--accent-red);
                        display:flex;align-items:center;justify-content:center;font-weight:800;color:#fff;font-size:0.9rem;flex-shrink:0;">
                        ${escapeHtml(name.charAt(0).toUpperCase())}
                    </div>
                    <div style="min-width:0;">
                        <p style="font-weight:700;font-size:0.9rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(name)}</p>
                        <p style="font-size:0.78rem;color:var(--text-secondary);">${escapeHtml(u.email || '—')} · <span class="badge badge-blue" style="font-size:0.7rem;">${escapeHtml(u.role || '—')}</span></p>
                        <p style="margin-top:4px;font-size:0.78rem;">Last reduced: ${trustLastReducedLabel(u.trustScoreLastReduced)}</p>
                    </div>
                </div>
                <div style="display:flex;align-items:center;gap:10px;flex-shrink:0;">
                    <div style="text-align:center;">
                        <div style="font-size:1.5rem;font-weight:900;color:var(--accent-red);line-height:1;">${score}</div>
                        <div style="font-size:0.68rem;color:var(--text-secondary);font-weight:600;">TRUST</div>
                    </div>
                    <button class="table-action-btn" style="background:var(--accent-blue);color:#fff;"
                        data-set-trust="${u.id}" data-name="${escapeHtml(name)}" data-score="${score}">
                        <i class="fa-solid fa-pen"></i> Set Score
                    </button>
                </div>
            `;
            lowTrustList.appendChild(card);
        });

        lowTrustList.querySelectorAll('[data-set-trust]').forEach(btn => {
            btn.addEventListener('click', () => openSetTrustModal(btn.dataset.setTrust, btn.dataset.name, parseInt(btn.dataset.score, 10)));
        });
    }
